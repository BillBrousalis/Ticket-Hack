...
int main()
{
  ...
  dev.get_firmware_version(buf);
  hprint(buf, "[+] Firmware Version: ", ':', FIRMWARE_VERSION_LEN);
  std::cout << std::endl;

  dev.read_passive_target(buf);
  hprint(buf, "[+] Card UID: ", '-', UID_LENGTH);
  std::cout << std::endl;

  std::cout << "[-Dumping Memory-]" << std::endl;
  for(int page=0; page<MAXPAGES; ++page) {
    dev.ultralight_read_page(buf, page);
